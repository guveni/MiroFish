...
Reads nodes from a Zep graph and keeps nodes matching predefined entity types.
"""
...
class ZepEntityReader:
    ...
    2. Keep nodes matching predefined entity types (labels beyond Entity).
    3. Attach related edges and related node information for each entity.
    """
    ...
    def _call_with_retry(
        ...
        """
        Call a Zep API operation with retry.
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Fetch all edges related to a node, with retry.
    ...
    def filter_defined_entities(
        ...
        Filtering rules:
        - Skip nodes whose labels only contain default Entity/Node labels.
        - Keep nodes with labels beyond Entity/Node.
        
        Args:
            graph_id: Graph ID.
            defined_entity_types: Optional entity type allowlist.
        ...
        if enrich_with_edges:
            for edge in all_edges:
                outgoing_edges.setdefault(edge["source_node_uuid"], []).append(edge)
                incoming_edges.setdefault(edge["target_node_uuid"], []).append(edge)
        ...
        for node in all_nodes:
            ...
            if enrich_with_edges:
                ...
                for edge in outgoing_edges.get(node["uuid"], []):
                ...
                for edge in incoming_edges.get(node["uuid"], []):
                    related_edges.append({
                        "direction": "incoming",
                        "edge_name": edge["name"],
                        "fact": edge["fact"],
        ...
        logger.info(
            f"Entity filtering complete: total_nodes={total_count}, "
            f"filtered={len(filtered_entities)}, entity_types={entity_types_found}"
        )
    ...
    def get_entity_with_context(
        ...
        """
        Fetch one entity with full edge and related-node context.
    ...
    def get_entities_by_type(
        ...
        """
        Fetch all entities of one type.
