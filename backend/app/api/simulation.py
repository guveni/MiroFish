...
# Adding this prefix avoids Agent calling tools and directly replies with text
...
def get_graph_entities(graph_id: str):
    ...
    Query parameters:
        entity_types: Comma-separated list of entity types (optional, used for further filtering)
...
def create_simulation():
    ...
    Note: max_rounds and other parameters are intelligently generated by the LLM, no need to set manually.
    
    Request (JSON):
        {
            "project_id": "proj_xxxx",      // Required
            "graph_id": "mirofish_xxxx",    // Optional, if not provided, retrieved from project
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: The running script (run_*.py) remains in the backend/scripts/ directory and is no longer copied to the simulation directory.
    ...
    try:
        ...
        # If config_generated=True and the files exist, it is considered prepared.
        # The following statuses all indicate that preparation is complete:
        # - ready: Preparation completed, ready to run
        # - preparing: If config_generated=True, it means completed
        # - running: Running, meaning preparation was completed long ago
        # - completed: Completed running, meaning preparation was completed long ago
        # - stopped: Stopped, meaning preparation was completed long ago
        # - failed: Run failed (but preparation is completed)
...
def prepare_simulation():
    ...
    Features:
    - Automatically detect completed preparation work to avoid redundant generation
    - If preparation is already complete, directly return the existing result
    ...
    try:
        ...
        try:
            ...
            reader = ZepEntityReader()
            # Fast-read entities (no edge info required, only count)
            filtered_preview = reader.filter_defined_entities(
                graph_id=state.graph_id,
                defined_entity_types=entity_types_list,
                enrich_with_edges=False  # Do not retrieve edge info to speed up
            )
        ...
        # Update simulation status (including pre-retrieved entity count)
...
def get_prepare_status():
    ...
    1. Query the ongoing task progress via task_id
    2. Check if there is already completed preparation work via simulation_id
    ...
    try:
        ...
        if not task_id:
            if simulation_id:
                # simulation_id exists but preparation is not complete
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    # __file__ is app/api/simulation.py, needs to go up two levels to backend/
...
def get_simulation_history():
    ...
    Used to display historical projects on the home page, returning a simulation list containing rich information such as project names and descriptions.
    ...
    try:
        ...
        # Enhance simulation data, reading only from Simulation files
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    - Directly reads files without passing through SimulationManager
    - Suitable for real-time viewing during generation
    - Returns extra metadata (such as file modification time, whether generation is in progress, etc.)
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns extra metadata (such as file modification time, whether generation is in progress, etc.)
    - Even if the configuration is not completely generated, partial information can be returned
...
def get_simulation_config(simulation_id: str):
    ...
    Returns containing:
        ...
        - agent_configs: Activity configuration of each Agent (activity level, posting frequency, stance, etc.)
...
def download_simulation_script(script_name: str):
    ...
    Available values for script_name:
    ...
    Request (JSON):
        {
            ...
            "force": false                         // Optional: force restart (will stop running simulations and clean up logs)
        }

    About the force parameter:
        - When enabled, if the simulation is running or completed, it will first stop and clean up running logs
        - Cleaned-up content includes: run_state.json, actions.jsonl, simulation.log, etc.
        - It will not clean up the configuration file (simulation_config.json) and profile files
        - Suitable for scenarios where the simulation needs to be rerun

    About enable_graph_memory_update:
        - When enabled, all Agent activities in the simulation (posting, commenting, liking, etc.) will be updated to Zep graph in real-time
        ...
        - Uses a batch update mechanism to reduce API call counts
    ...
    Response:
        {
            ...
            "data": {
                ...
                "force_restarted": true               // Whether it is a forced restart
            }
        }
    """
    try:
        ...
        enable_graph_memory_update = data.get('enable_graph_memory_update', False)  # Optional: whether to enable graph memory updates
        ...
        # Smart handling of status: if preparation is completed, allow restart
        if state.status != SimulationStatus.READY:
            # Check if preparation work is completed
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            ...
            if is_prepared:
                ...
                # Process does not exist or has finished, reset status to ready
        ...
        # Get graph ID (used for graph memory updates)
        graph_id = None
        if enable_graph_memory_update:
            # Retrieve graph_id from simulation status or project
            graph_id = state.graph_id
            if not graph_id:
                # Attempt to retrieve from project
                project = ProjectManager.get_project(state.project_id)
                if project:
...
def get_run_status_detail(simulation_id: str):
    ...
    Used to display real-time dynamics on the frontend.
    ...
    try:
        ...
        # Get actions of the current round (recent_actions only displays the latest round)
        ...
        # recent_actions only displays the content of both platforms in the latest round
...
def get_simulation_actions(simulation_id: str):
    ...
    Query parameters:
        limit: Returned count (default 100)
...
def get_simulation_timeline(simulation_id: str):
    ...
    Used to display progress bars and timeline views on the frontend.
    
    Query parameters:
        start_round: Starting round (default 0)
        end_round: Ending round (default all)
    
    Returns summarized information of each round.
    """
    try:
...
def get_agent_stats(simulation_id: str):
    ...
    Used to display Agent activity ranking, action distribution, etc., on the frontend.
    """
    try:
...
def get_simulation_posts(simulation_id: str):
    ...
    Query parameters:
        ...
        limit: Returned count (default 50)
        offset: Offset
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be in a running state (entering waiting command mode after completing the simulation loop).

    Request (JSON):
        {
            "simulation_id": "sim_xxxx",       // Required, Simulation ID
            "agent_id": 0,                     // Required, Agent ID
            "prompt": "What is your opinion on this matter?",  // Required, interview prompt
            "platform": "twitter",             // Optional, specify platform (twitter/reddit)
                                               // When unspecified: dual-platform simulation interviews both platforms simultaneously
            "timeout": 60                      // Optional, timeout (seconds), default 60
        }

    Response (unspecified platform, dual-platform mode):
        {
            "success": true,
            "data": {
                "agent_id": 0,
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, interview list
                {
                    "agent_id": 0,
                    "prompt": "What are your thoughts on A?",
                    "platform": "twitter"      // Optional, specify the interview platform for this Agent
                },
                {
                    "agent_id": 1,
                    "prompt": "What are your thoughts on B?"  // If platform is not specified, use the default
                }
            ],
            "platform": "reddit",              // Optional, default platform (overridden by individual platform fields)
                                               // When unspecified: dual-platform simulation interviews each Agent on both platforms simultaneously
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            # Validate platform of each item (if any)
        ...
        # Optimize prompt of each interview item, add prefix to prevent Agent from calling tools
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required, Simulation ID
            "prompt": "What is your overall opinion on this matter?",  // Required, interview prompt (all Agents use the same prompt)
...
def get_interview_history():
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",          // Optional, platform type (reddit/twitter)
                                           // If unspecified, return all history of both platforms
            "agent_id": 0,                 // Optional, get only the interview history of this Agent
            "limit": 100                   // Optional, returned count, default 100
        }
    ...
    try:
        ...
        simulation_id = data.get('simulation_id')
        platform = data.get('platform')  # If unspecified, returns history from both platforms
        agent_id = data.get('agent_id')
...
def close_simulation_env():
    ...
    Send close environment command to simulation, making it exit command mode gracefully.
    
    Note: This is different from the /stop endpoint, /stop will force-terminate the process,
    while this endpoint will make the simulation gracefully close the environment and exit.
