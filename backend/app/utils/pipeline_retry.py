...
async def run_pipeline_step_async(
    ...
    *,
    ...
    """
    Async variant of run_pipeline_step.

    ``fn`` may return either a direct value or an awaitable. Retries use
    ``asyncio.sleep`` so concurrent I/O tasks can keep making progress.
    """
    skip_msgs = frozenset(non_retryable_value_messages or ())
    max_r = (
        Config.PIPELINE_STEP_MAX_RETRIES if max_retries is None else max_retries
